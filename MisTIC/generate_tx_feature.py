...
def expression_feature(adata: sc.AnnData,
    ...
    with process_time_ram("Performing one-vs-rest differential analysis...") as ctm:
        ...
        for celltype in tqdm(adata.uns["unique_leiden"]):
            # Since deseq2 will replace _ with - -- actually not with the newer version of deseq2
            #ct = celltype.replace("_", "-")
            ct = celltype
            print("Julia Modification -- not replacing _ with -")
            ...
            stat_res = DeseqStats(dds, 
                                inference=inference, 
                                contrast=['leiden', "cell_type_m_"+ct, ct],
                                quiet=True)
            stat_res.summary() 
