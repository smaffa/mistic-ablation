...
def import_data(cell_metadata: Union[str, pd.DataFrame],
    ...
    with process_time_ram("Processing cell boundaries") as ctm:
        ...
        # REMOVED: assert set(cell_coords.index) == set(cell_meta.index)
        # Subsetting is deferred to after tx_metadata is loaded (see below)
    ...
    with process_time_ram("Processing Transcript information ") as ctm:
        ...
        # REMOVED: assert set(tx_metadata['cell_id']) == set(cell_meta.index)
        # ADDED: Subset cell_meta and cell_coords to the cells present in tx_metadata.
        # tx_metadata cells are assumed to be a subset of those in cell_meta and cell_coords.
        print("Subsetting to just cells in tx_metadata\n")
        tx_cells = set(tx_metadata['cell_id'])
        cell_meta = cell_meta.loc[cell_meta.index.isin(tx_cells)]
        cell_coords = cell_coords.loc[cell_coords.index.isin(tx_cells)]
    ...
    with process_time_ram("Processing cell by gene matrix") as ctm:
        ...
        # ADDED: Subset counts to tx_cells for consistency when cell_by_gene_counts is provided externally
        counts = counts.loc[counts.index.isin(tx_cells)]
        # Sanity check 
...
def generate_count_patches(adata: Union[sc.AnnData, pl.DataFrame],
    ...
    print("tx_to_remove_cols2",tx_to_remove.columns)
    print("tx_to_reassign_cols2",tx_to_reassign.columns)
    print("tx_to_remove type",type(tx_to_remove))
    if "cell_id" not in tx_to_reassign.columns:
        if "from_cell_id" in tx_to_reassign.columns:
            tx_to_reassign["cell_id"] = tx_to_reassign["from_cell_id"]
    if "cell_id" not in tx_to_remove.columns:
        if "from_cell_id" in tx_to_remove.columns:
            print("right before type tx_to_remove", type(tx_to_remove))
            print("right before tx_to_remove columns", tx_to_remove.columns)
            tx_to_remove = tx_to_remove.with_columns(
                pl.col("from_cell_id").alias("cell_id")
            )
    ...
    print("cell to remove columns",cell_to_remove.columns)
